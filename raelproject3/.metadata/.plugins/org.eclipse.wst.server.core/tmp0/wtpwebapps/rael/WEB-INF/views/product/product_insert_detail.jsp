    	...
    	<!-- End of Topbar -->
    		<div class="card shadow mb-4">
				<div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
					<h6 class="m-0 font-weight-bold text-primary">상품 추가 등록 양식</h6>
				</div>
				<div class="card-body">
					<p>
						<code>등록 되어 있는 상품 번호의 다른 종류에 상품 일 경우</code> 추가 등록 탭에서 진행.
						<div class="my-2"></div>
							EX. <code>같은 상품</code>이지만 <code>다른 색상</code>일 경우, <code>무게</code> 및 
							<code>가격</code> 등이 달라 추가로 등록 해야 하는 경우나 <code>판매처</code>가 달라 <code>재고</code>를 따로 관리해야 하는 경우.
						<div class="my-2"></div>
							우선 추가 등록할 상품을 선택 하고서 나머지 항목을 입력하고 난 후 제출하면 등록 완료!
							
						<form method="POST" action="/product/product_insert_detail/INSERT">
							<ul>
									<div class="input-group">
										<select name="productBox" id="productBox">
											<option value="defaultValue" selected></option>
											<c:forEach var="list" items="${productdatas}">
												<option value="${list}">
													${list}
												</option>
											</c:forEach>
