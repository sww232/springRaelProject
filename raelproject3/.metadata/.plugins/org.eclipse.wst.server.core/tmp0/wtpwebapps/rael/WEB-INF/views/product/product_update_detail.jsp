													...
													<c:forEach items="${datas.list}" var="data">
														...
														<tr>
															<td>
																...
																value="${data.PRODUCTNUM} / ${data.PRODUCTBYNUM}" 
																formaction="/product/product_update_detail/${data.PRODUCTNUM}/${data.PRODUCTBYNUM}">
																	<span class="icon text-white-50">
									...
									<ul class="pagination">
									    ...
									    <c:if test="${datas.pagination.existPrevPage eq true}">
									    	<li class="paginate_button page-item previous">
									    		<a aria-controls="dataTable" data-dt-idx="${num}" class="page-link"
									    			href='<c:url value="/product/product_update_detail?categoryBox=${categoryBox}&page=${datas.pagination.startPage-1}&recordSize=10&pageSize=10"/>'>PREVIOUS</a>
									    	</li>
									    </c:if>
									    <c:if test="${datas.pagination.totalPageCount > 0}">
									   		<c:forEach begin="${datas.pagination.startPage}" end="${datas.pagination.endPage}" var="num">
									   			<li class="paignate_button page-item">
									   				<a  aria-controls="dataTable" data-dt-idx="${num}" class="page-link"
									   					href='<c:url value="/product/product_update_detail?categoryBox=${categoryBox}&page=${num}&recordSize=10&pageSize=10"/>'>${num}</a>
									   			</li>
									   		</c:forEach>
									    </c:if>
									    <c:if test="${datas.pagination.existNextPage eq true}">
									        <li class="paginate_button page-item next" id="dataTable_next">
									        	<a aria-controls="dataTable" data-dt-idx="${num}" class="page-link"
									        		href='<c:url value="/product/product_update_detail?categoryBox=${categoryBox}&page=${datas.pagination.endPage+1}&recordSize=10&pageSize=10"/>'>NEXT</a>
									       </li>
									    </c:if>
						    		</ul>
					    		</div>
