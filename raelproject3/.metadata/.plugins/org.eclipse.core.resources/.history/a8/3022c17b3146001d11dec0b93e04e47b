											...
											<table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
												...
												<tbody>
													<c:forEach items="${datas.list}" var="data">
														<input type="hidden" name="product name for modify" value="${data.PRODUCTNAME}">
														<tr>
															<td>
																<button type="submit" class="btn btn-secondary btn-icon-split" 
																value="${data.PRODUCTNUM}" 
																formaction="/product/product_update/${data.PRODUCTNUM}">
																	<span class="icon text-white-50">
																		<i class="fas fa-arrow-right"></i>
																	</span>
																	<span class="text">${data.PRODUCTNUM}</span>
																</button>
															</td>
											...
											<ul class="pagination">
											    ...
											    <c:if test="${datas.pagination.existPrevPage eq true}">
											    	<li class="paginate_button page-item previous">
											    		<a aria-controls="dataTable" data-dt-idx="${num}" class="page-link"
											    			href='<c:url value="/product/product_update?categoryBox3=${categoryBox}&page=${datas.pagination.startPage-1}&recordSize=10&pageSize=10"/>'>PREVIOUS</a>
											    	</li>
											    </c:if>
											    <c:if test="${datas.pagination.totalPageCount > 0}">
											   		<c:forEach begin="${datas.pagination.startPage}" end="${datas.pagination.endPage}" var="num">
											   			<li class="paignate_button page-item">
											   				<a  aria-controls="dataTable" data-dt-idx="${num}" class="page-link"
											   					href='<c:url value="/product/product_update?categoryBox3=${categoryBox}&page=${num}&recordSize=10&pageSize=10"/>'>${num}</a>
											   			</li>
											   		</c:forEach>
											    </c:if>
											    <c:if test="${datas.pagination.existNextPage eq true}">
											        <li class="paginate_button page-item next" id="dataTable_next">
											        	<a aria-controls="dataTable" data-dt-idx="${num}" class="page-link"
											        		href='<c:url value="/product/product_update?categoryBox3=${categoryBox}&page=${datas.pagination.endPage+1}&recordSize=10&pageSize=10"/>'>NEXT</a>
											       </li>
											    </c:if>
								    		</ul>
							    		</div>
