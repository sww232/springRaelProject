...
<form>
	...
	<div id="wrapper">
		...
		<!-- Page Heading -->
		<h1 class="h3 mb-2 text-gray-800">카테고리 수정 탭</h1>
			...
			<div class="card-body">
				<div class="table-responsive">
					<div id="dataTable_wrapper" class="dataTables_wrapper dt-bootstrap4">
							...
							<form>
								<div class="dataTable_filter" id="dataTable_filter">
									<select name="categoryBox" id="categoryBox">
										<option value="defaultValue" selected></option>
										<c:forEach var="list" items="${categorynums2}">
											<option value="${list}">
												${list}
											</option>
										</c:forEach>
						...
						<div class="row">
							<div class="col-sm-12">
								<form method="POST" action="/product/category_update/UPDATE">
								<table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
									...
									<tbody>
										<c:forEach items="${datas.list}" var="data">
											<input type="hidden" value=${data.PRODUCTCATEGORYNUM} 
												class="form-control bg-light border-0 small" name="product category num for modify" readonly>
											<tr>
												<td>${data.PRODUCTCATEGORYNUM}</td>
												<td><input type="text" value="${data.PRODUCTCATEGORYNAME}" 
												class="form-control bg-light border-0 small" name="product category name for modify"> </td>
											<tr>
										</c:forEach>
									</tbody>
								</table>
									...
									<ul class="pagination">
									    ...
									    <c:if test="${datas.pagination.existPrevPage eq true}">
									    	<li class="paginate_button page-item previous">
									    		<a aria-controls="dataTable" data-dt-idx="${num}" class="page-link"
									    			href='<c:url value="/product/category_update?page=${datas.pagination.startPage-1}&recordSize=10&pageSize=10&categoryBox=${categoryBox}"/>'>PREVIOUS</a>
									    	</li>
									    </c:if>
									    <c:if test="${datas.pagination.totalPageCount > 0}">
									   		<c:forEach begin="${datas.pagination.startPage}" end="${datas.pagination.endPage}" var="num">
									   			<li class="paignate_button page-item">
									   				<a  aria-controls="dataTable" data-dt-idx="${num}" class="page-link"
									   					href='<c:url value="/product/category_update?page=${num}&recordSize=10&pageSize=100&categoryBox=${categoryBox}"/>'>${num}</a>
									   			</li>
									   		</c:forEach>
									    </c:if>
									    <c:if test="${datas.pagination.existNextPage eq true}">
									        <li class="paginate_button page-item next" id="dataTable_next">
									        	<a aria-controls="dataTable" data-dt-idx="${num}" class="page-link"
									        		href='<c:url value="/product/category_update?page=${datas.pagination.endPage+1}&recordSize=10&pageSize=10&categoryBox=${categoryBox}"/>'>NEXT</a>
									       </li>
									    </c:if>
						    		</ul>
					    		</div>
