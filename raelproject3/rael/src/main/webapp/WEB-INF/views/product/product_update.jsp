		...
		<div id="content-wrapper" class="d-flex flex-column">
			...
			<div id="content">
				...
				<div class="container-fluid">
					<!-- Page Heading -->
					<h1 class="h3 mb-2 text-gray-800">상품명 변경 탭</h1>
					<button class="btn btn-info btn-icon-split" onclick="location.href='product_update_detail?categoryBox=${categoryBox}'">
						<span class="icon text-white-50"> <i class="fas fa-info-circle"></i>
						</span> <span class="text">상품 상세 변경</span>
					</button>
												...
												<table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
													...
													<tbody>
														<c:forEach items="${datas.list}" var="data">
															...
															<input type="hidden" name="product name for modify" value="${data.PRODUCTNAME}" readonly>
															<input type="hidden" name="product num for modify" value="${data.PRODUCTNUM}" readonly>
															<tr>
																<td>
																	<button type="submit" class="btn btn-secondary btn-icon-split" value="${data.PRODUCTNUM}" formaction="/product/product_update/${data.PRODUCTNUM}">
																		<span class="icon text-white-50"> <i class="fas fa-arrow-right"></i>
																		</span> <span class="text">${data.PRODUCTNUM}</span>
																	</button>
																</td>
																<td>${data.PRODUCTCATEGORYNUM}/${data.PRODUCTCATEGORYNAME}</td>
											...
											<div class="dataTables_paginate paging_simple_numbers" id="dataTable_paginate">
												<ul class="pagination">
													...
													<c:if test="${datas.pagination.existPrevPage eq true}">
														<li class="paginate_button page-item previous"><a aria-controls="dataTable" data-dt-idx="${num}" class="page-link" href='<c:url value="/product/product_update?categoryBox3=${categoryBox}&page=${datas.pagination.startPage-1}&recordSize=${recordSize}&pageSize=${pageSize}&keyword=${param.keyword}&searchType=${param.searchType}"/>'>PREVIOUS</a></li>
													</c:if>
													<c:if test="${datas.pagination.totalPageCount > 0}">
														<c:forEach begin="${datas.pagination.startPage}" end="${datas.pagination.endPage}" var="num">
															<c:if test="${param.page eq num}">
																<li class="paignate_button page-item active"><a aria-controls="dataTable" data-dt-idx="${num}" class="page-link" href='<c:url value="/product/product_update?categoryBox3=${categoryBox}&page=${num}&recordSize=${recordSize}&pageSize=${pageSize}&keyword=${param.keyword}&searchType=${param.searchType}"/>'>${num}</a></li>
															</c:if>
															<c:if test="${param.page ne num}">
																<li class="paignate_button page-item"><a aria-controls="dataTable" data-dt-idx="${num}" class="page-link" href='<c:url value="/product/product_update?categoryBox3=${categoryBox}&page=${num}&recordSize=${recordSize}&pageSize=${pageSize}&keyword=${param.keyword}&searchType=${param.searchType}"/>'>${num}</a></li>
															</c:if>
															
														</c:forEach>
													</c:if>
													<c:if test="${datas.pagination.existNextPage eq true}">
														<li class="paginate_button page-item next" id="dataTable_next"><a aria-controls="dataTable" data-dt-idx="${num}" class="page-link" href='<c:url value="/product/product_update?categoryBox3=${categoryBox}&page=${datas.pagination.endPage+1}&recordSize=${recordSize}&pageSize=${pageSize}&keyword=${param.keyword}&searchType=${param.searchType}"/>'>NEXT</a></li>
													</c:if>
												</ul>
											</div>
										</div>
